/*Programme:2corps-Soleil_Terre(Runge-Kutta)*/
#include <stdio.h>
#include <math.h>
...
/*Resolution d'equation differentielle par Runge-Kutta*/
void rk(double r[],double dr[],double dt){
	double k1[12],k2[12],k3[12],k4[12],d1[12],d2[12];
	int i;
	for(i=0;i<12;i++){
		k1[i]=dt*dr[i];			/*k1*/
		d1[i]=r[i]+0.5*k1[i];	/*r+0.5k1*/
	}
	f(d1,d2);	/*dr de r+0.5k1*/

	for(i=0;i<12;i++){  
		k2[i]=dt*d2[i];			/*k2*/
		d1[i]=r[i]+0.5*k2[i];	/*r+0.5k2*/
	}
	f(d1,d2);	/*dr de r+0.5k2*/
...
{
	FILE*fich0;
	fich0=fopen("rk2.txt","w");		/*Positions*/
	FILE*fich1;
	fich1=fopen("extra2rk.txt","w");	/*r(t),impulsions,moment cinétique total,énergie total*/
	FILE*fich3;
	fich3=fopen("pe2rk.txt","w");		/*Périhélies*/
